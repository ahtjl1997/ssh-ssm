...
Member m = (Member)dao.findById(member.getId(),"Member");
 %>
	...
	<div class="fn-clear"></div>
		...
		<tbody>
<%
String hql="from Score where saleid='"+m.getId()+"'";
String url="pingfenlist2.jsp?1=1";
String key = Info.getUTFStr(request.getParameter("key"));
if(key!=null){
hql+=" and memberid in (select id from Member where uname like '%"+key+"%')";
url+="&key="+key;
}
...
Member mmm = (Member)dao.findById(s.getMemberid(),"Member");
%>					 
							<tr>
							    <td><%=mmm.getUname() %></td>
							    <td><%=s.getScore() %></td>
